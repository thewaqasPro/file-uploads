...
interface MediaLibraryProps {
  isOpen?: boolean; // Controls the visibility of the dialog (optional for standalone mode)
  onClose?: () => void; // Callback function when the dialog is closed (optional for standalone mode)
  onImageSelect: (imageUrl: string) => void; // Callback function when an image is selected
  isStandalone?: boolean; // New prop: if true, renders directly, not in a dialog
}

const IMAGES_PER_LOAD = 15; // Define how many images to fetch per "load more"

export function MediaLibrary({
  isOpen = false, // Default to false if not provided (for dialog mode)
  onClose = () => {}, // Default empty function if not provided
  onImageSelect,
  isStandalone = false, // Default to false (dialog mode)
}: MediaLibraryProps) {
  ...
  // Effect hook to fetch initial images when the dialog opens or component mounts (for standalone)
  useEffect(() => {
    if (isStandalone || isOpen) {
      // Fetch if standalone or if dialog is open
      // Reset state when opening the modal or mounting standalone to fetch from scratch
      setImages([]);
      setOffset(0);
      setHasMore(true);
      fetchImages(0, false); // Fetch initial set, do not append
    }
  }, [isStandalone, isOpen, fetchImages]);
  ...
  if (isStandalone) {
    return content; // Render content directly if in standalone mode
  }

  // Otherwise, render within the Dialog components
  return (
